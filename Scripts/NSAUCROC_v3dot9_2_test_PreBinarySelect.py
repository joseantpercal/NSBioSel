...
def myAUCROCFeatureSelection(adata, df_dummies, cl, n_top_genes, binary_dummies):
    ...
    print("\t", "Pre-selected", x_train.shape[1], "genes to feed into AUCROC.")
    ...
    df_scores = pd.Series(feature_dict.values(), index=feature_dict.keys()).sort_values(ascending=False)[:n_top_genes]

    #df_scores = pd.Series(feature_scores[auc], index=feature_scores[col]).sort_values(ascending=False).reset_index(drop=True)[:n_top_genes]
...
def myDecisionTreeEvaluationTest(adata, adata_test, cluster_header, NSForest_results, beta, output_folder, outputfilename_prefix):
    ...
    # Datos de ejemplo
    genes_data = NSForest_results['NSAUCROC_markers']
...
def NSAUCROC(adata, cluster_header, medians_header, binary_scores_header,
    ...
    for cl in tqdm(cluster_list, desc = "running AUCROC on all clusters"): 
        ...
        ## AUCROC step: get top genes ranked correlation between variables and target
        top_ROC_genes = myAUCROCFeatureSelection(adata, df_dummies, cl, n_top_genes, binary_dummies)
       ...
       ## filter out negative genes by thresholding median>0 ==> to prevent deviding by 0 in binary score calculation
        ...
        if n_positive_genes < n_binary_genes:
            print("\t" + f"Only {n_positive_genes} out of {n_top_genes} top ROC features with median > 0 will be further evaluated.")
        ...
        top_binary_genes = pd.Series(binary_scores.loc[cl], index=top_ROC_genes_positive.index).sort_values(ascending=False)
        ...
        df_supp_cl = pd.DataFrame({'clusterName': cl,
                                   ...
                                   'rf_feature_importance': top_ROC_genes[binary_genes_list],
    ...
    print("Tiempo total de la ejecución: {:.2f} segundos".format(elapsed_total_time))    ### END iterations ###
