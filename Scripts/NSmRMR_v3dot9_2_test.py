...
## run mRMR on the binary dummy variables ==> outputs all genes ranked by Gini impurit
def myMRMR(adata, df_dummies, cl, n_top_genes):

    start_time = time.time()  # Registrar el tiempo de inicio

    # Convertir adata a DataFrame
    x_train = adata.to_df()
    # Obtener la variable objetivo
    y_train = df_dummies[cl]
    
    # Seleccionar las características utilizando mRMR
    selected_features = mrmr_classif(X=x_train, y=y_train, K=n_top_genes)
    
    # Obtener los índices numéricos correspondientes a las columnas seleccionadas
    selected_feature_indices = [x_train.columns.get_loc(col) for col in selected_features]
    
    # Obtener los nombres de las características seleccionadas
    selected_feature_names = x_train.columns[selected_feature_indices]
    
    # Calcular la importancia de las características seleccionadas
    feature_importance = [x_train[col].corr(y_train) for col in selected_feature_names]
    
    # Crear una serie con la importancia de las características, indexada por los nombres de las características
    top_mrmr_genes = pd.Series(feature_importance, index=selected_feature_names).sort_values(ascending=False)[:n_top_genes]
...
## construct decision tree for each gene and evaluate the fbeta score in all combinations ==> outputs markers with max fbeta, and all scores
def myDecisionTreeEvaluationTest(adata, adata_test, cluster_header, NSmRMR_results, beta, output_folder, outputfilename_prefix):
    
    # Datos de ejemplo
    genes_data = NSmRMR_results['NSmRMR_markers']
    cluster_names = NSmRMR_results['clusterName']
    ...
    ## dummy/indicator for one vs. all mRMR model
    ...
    results_df = pd.DataFrame(columns=['clusterName', 'clusterSize', 'f_score', 'PPV', 'TN', 'FP', 'FN', 'TP', 'marker_count', 'NSmRMR_markers', 'threshold'])
    ...
    #         x_train = adata[:,i].X
        ...
        dict_scores_test['NSmRMR_markers'] = str(gene_list)
...
## construct decision tree for each gene and evaluate the fbeta score in all combinations ==> outputs markers with max fbeta, and all scores
def myDecisionTreeEvaluationTestCombined(adata, adata_test, cluster_header, NSmRMR_results, beta, output_folder, outputfilename_prefix):
    ...
    for cl, genes in cluster_gene_pairs:  
        ...
        dict_scores_test['NSmRMR_markers'] = str(genes)
...
def myDecisionTreeEvaluationTestMove(adata, adata_test, cluster_header, NSmRMR_results, beta, output_folder, outputfilename_prefix, coef=0.5):
    ...
    # DataFrame para almacenar los resultados
    results_df = pd.DataFrame(columns=['clusterName', 'clusterSize', 'f_score', 'PPV', 'TN', 'FP', 'FN', 'TP', 'marker_count', 'NSmRMR_markers', 'original_threshold', 'new_threshold'])
...
def NSMRMR(adata, cluster_header, cluster_list=None, medians_header=None,
    ...
    for cl in cluster_list:
        ...
        ## mRMR step: get top genes ranked by feature importance
        top_rf_genes = myMRMR(adata, df_dummies, cl, n_top_genes)
        ...
        if n_positive_genes < n_binary_genes:
            print("\t" + f"Only {n_positive_genes} out of {n_top_genes} top mRMR features with median > 0 will be further evaluated.")
        ...
        df_supp_cl = pd.DataFrame({'clusterName': cl,
                                   ...
                                   'mRMR_feat_imp': top_rf_genes[binary_genes_list],
